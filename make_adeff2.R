...
adeff <- data %>%
  mutate(trtgrp = factor(trtgrp.x, level = TLevels),
         das28crp_BL = if_else(is.na(das28crp_BL), das28_BL, das28crp_BL)) %>%
  rename(subjid = patid.x, bldt = bldt.x) %>%
  select(-trtgrp.x)

temp <- adeff %>% select(tcid, trtgrp) %>% unique()

adeff <- adeff  %>% select(-subjid, -bldt, -trtgrp) %>% 
  group_by(tcid) %>% 
